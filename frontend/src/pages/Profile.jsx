...
import { Settings, MapPin, Calendar, Tag, Save, UserCircle, Cake, Hash, Users, Camera, LogOut, ShieldCheck, Trash2 } from 'lucide-react';
...
const Profile = () => {
  ...
  const [userProfile, setUserProfile] = useState({
    name: "", clubId: "", location: "", birthDate: "", age: "", gender: "", joined: "May 2026", 
  });
  ...
  // RESTORED: State to hold the user's fetched items
    ...
    const fetchProfileData = async () => {
      if (user && token) {
        try {
          ...
          // 2. RESTORED: Fetch User's Marketplace Listings
          const resItems = await fetch(`${API_URL}/api/items/my-items`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          if (resItems.ok) {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#0B0F19] text-slate-100 flex flex-col items-center justify-center px-6 text-center">
        ...
        <h1 className="text-3xl md:text-5xl font-black text-white uppercase tracking-tight mb-4">Access Restricted</h1>
        <p className="text-slate-400 text-lg max-w-md mb-8">You must log in to your account to create an official CNPC Digital ID and manage your marketplace listings.</p>
        <button onClick={() => navigate('/auth')} className="bg-gradient-to-r from-cnpc-accent to-lime-400 text-slate-950 px-10 py-4 rounded-full font-black uppercase tracking-widest hover:brightness-110 transition-all shadow-[0_0_20px_rgba(196,214,0,0.3)]">
          Go to Login
        </button>
      </div>
    );
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'name' || name === 'location') updatedValue = value.toUpperCase();
    const updatedUser = { ...userProfile, [name]: updatedValue };
    if (name === 'birthDate') updatedUser.age = calculateAge(updatedValue);
            ...
            <form onSubmit={handleSaveProfile} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input required name="name" value={userProfile.name} onChange={handleInputChange} type="text" placeholder="E.G. JUAN DE LA CRUZ" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white uppercase focus:outline-none focus:border-cnpc-accent transition-colors" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Location / City</label>
                  <input required name="location" value={userProfile.location} onChange={handleInputChange} type="text" placeholder="E.G. DAET, CAMARINES NORTE" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white uppercase focus:outline-none focus:border-cnpc-accent transition-colors" />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Birth Date</label>
                  <input required name="birthDate" value={userProfile.birthDate} onChange={handleInputChange} type="date" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cnpc-accent transition-colors [color-scheme:dark]" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Age</label>
                  <input readOnly name="age" value={userProfile.age} type="number" placeholder="Auto-calculated" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-slate-400 cursor-not-allowed focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Gender</label>
                  <select required name="gender" value={userProfile.gender} onChange={handleInputChange} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cnpc-accent transition-colors appearance-none">
              ...
              <div className="flex flex-col-reverse sm:flex-row justify-between items-center pt-4 gap-4">
                ...
                <button type="submit" className="w-full sm:w-auto bg-gradient-to-r from-cnpc-accent to-lime-400 text-slate-950 px-8 py-4 rounded-xl font-bold uppercase tracking-widest hover:brightness-110 transition-all flex items-center justify-center gap-2">
                  <Save className="w-5 h-5" /> Save & Generate ID
                </button>
              </div>
            </form>
          </div>
            ...
            <div className="relative bg-white/5 border border-white/10 rounded-3xl p-8 md:p-12 overflow-hidden shadow-2xl mb-8 group animate-in fade-in duration-500">
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-8">
                ...
                <div className="flex-1 text-center md:text-left">
                  ...
                  <h1 className="text-3xl md:text-5xl font-black text-white uppercase tracking-tight mb-4">{userProfile.name}</h1>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    <div className="flex flex-col items-center md:items-start gap-1">
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Age</span>
                      <div className="flex items-center gap-1.5 text-slate-200 text-sm font-bold"><Hash className="w-4 h-4 text-cnpc-accent" />{userProfile.age} YRS</div>
                    </div>
                    <div className="flex flex-col items-center md:items-start gap-1">
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Gender</span>
                      <div className="flex items-center gap-1.5 text-slate-200 text-sm font-bold uppercase"><Users className="w-4 h-4 text-cnpc-accent" />{userProfile.gender}</div>
                    </div>
                    <div className="flex flex-col items-center md:items-start gap-1">
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">DOB</span>
                      <div className="flex items-center gap-1.5 text-slate-200 text-sm font-bold"><Cake className="w-4 h-4 text-cnpc-accent" />{new Date(userProfile.birthDate).toLocaleDateString()}</div>
                    </div>
                    <div className="flex flex-col items-center md:items-start gap-1">
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Joined</span>
                      <div className="flex items-center gap-1.5 text-slate-200 text-sm font-bold"><Calendar className="w-4 h-4 text-cnpc-accent" />{userProfile.joined}</div>
                    </div>
                  </div>
                  <div className="flex items-center justify-center md:justify-start gap-2 text-slate-400 font-medium text-sm border-t border-white/10 pt-4 mt-2">
                    <MapPin className="w-4 h-4 text-cnpc-accent" />{userProfile.location}
                  </div>
                </div>

                <div className="flex flex-row md:flex-col gap-3 w-full md:w-auto mt-4 md:mt-0">
                  <button onClick={() => setIsEditing(true)} className="flex-1 md:flex-none bg-white/10 hover:bg-white/20 text-white px-6 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors border border-white/5">
                    ...
                    {myListings.map((item) => (
                      <div key={item.id} className="flex flex-col sm:flex-row items-center justify-between p-6 rounded-2xl bg-black/20 border border-white/5 hover:border-white/10 transition-colors gap-6">
                        ...
                        <div className="flex items-center gap-3 w-full sm:w-auto">
                          <div className={`flex-1 sm:flex-none px-4 py-2 border rounded-lg text-xs font-bold uppercase tracking-widest text-center ${item.status === 'approved' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20'}`}>
                            {item.status}
                          </div>
                          {/* RESTORED: Delete Item Button */}
                          <button onClick={() => handleDeleteItem(item.id)} className="p-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg transition-colors border border-red-500/20">
